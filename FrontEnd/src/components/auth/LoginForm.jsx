    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 animate-fade-in-up">
            ...
            <div className="flex items-center justify-between text-xs px-1">
                <label className="flex items-center gap-2 cursor-pointer group text-snap-text-secondary">
                    <input
                        {...register('rememberMe')}
                        type="checkbox"
                        disabled={login.isPending}
                        className="w-4 h-4 rounded border-white/20 bg-white/5 accent-snap-cyan cursor-pointer disabled:opacity-50"
                    />
                    <span className="select-none">Remember me</span>
                </label>
                <button type="button" className="text-snap-cyan hover:underline transition-all font-medium">
                    Forgot password?
                </button>
            </div>
